...
**Дата:** 2026-08-09 (база), 2026-08-17 (полная реализация)  
**Задача:** PLAN_SUMMARY_v3.5.0.md #4  
**Статус:** ✅ Выполнено — полный пайплайн (парсинг → категоризация → сводка)
...
| Метрика | Значение |
|---------|----------|
| Создано файлов | 7 (репозитории, процессор, миграция, роут) |
| Моделей | 2 (WebSource, WebNews) |
| Статус | Полный пайплайн |

---

## 🚀 Обновление v4.2.0 (2026-08-17)

Добавлен полный пайплайн обработки Web новостей:

1. **`WebProcessorService`** — парсинг → `web_news` → `CategorizationQueue`
2. **Scheduler `_run_web_parser()`** — автоматический запуск каждые 5 минут
3. **Web Admin API `/web/`** — CRUD источников, ручной парсинг, список новостей
4. Web новости проходят через ту же очередь категоризации, что и Telegram и RSS
5. Включаются в мульти-источниковые сводки через Orchestrator
